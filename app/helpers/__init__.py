...
def modethumb(name):
    ...
    if os.path.isfile(os.path.join(basedir, 'app','static','img','modethumbs', name + '.png')):
        return flask.url_for('static', filename='img/modethumbs/' + name + '.png', external=True)
    else:
        return flask.url_for('static', filename='img/modethumbs/othermode.png', external=True)
